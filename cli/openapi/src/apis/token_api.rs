...
/// struct for typed errors of method [`remove_token`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum RemoveTokenError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`token_info`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn list_tokens(configuration: &configuration::Configuration, ) -> Result<Vec<models::TokenResponse>, Error<ListTokensError>> {

    let uri_str = format!("{}/token/list", configuration.base_path);
...
pub async fn remove_token(configuration: &configuration::Configuration, remove_token_request: models::RemoveTokenRequest) -> Result<(), Error<RemoveTokenError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RemoveTokenError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn token_info(configuration: &configuration::Configuration, ) -> Result<models::User, Error<TokenInfoError>> {

    let uri_str = format!("{}/token/info", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<TokenInfoError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
