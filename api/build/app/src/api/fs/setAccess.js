...
export default async function setAccess(user, mnemonic, body) {
    const { sub } = user;
    const inode = await requireWrite(mnemonic, sub);
    const { members } = inode;
    const permission = members.find((m) => m.sub === body.sub)?.permission;
    if (!permission) {
        throw new RequestError(`User ${body.sub} is not a direct member of ${mnemonic}`);
    }
...
//# sourceMappingURL=setAccess.js.map