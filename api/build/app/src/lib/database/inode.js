...
import { InodeType } from 'src/api/types';
...
export const listFiles = async (mnemonic) => {
...
const getInodesRecursive = async (inode) => {
...
export const listTree = async (mnemonic) => {
...
const getParentsRecursive = async (inodeId) => {
    if (!inodeId) {
        return [];
    }
    const inode = await db.inode.findUnique({
...
export const getParents = async (mnemonic) => {
    const inode = await db.inode.findUnique({
        where: {
            mnemonic,
        },
...
const isChildOfRecursive = async (inodeId, parentId) => {
    if (inodeId === null) {
        return false;
    }
    const inode = await db.inode.findUnique({
...
export const isChildOf = async (mnemonic, parent) => {
    const inode = await db.inode.findUnique({
        where: {
            mnemonic,
        },
