...
export function ApiWrapper({ children }) {
  ...
  const admin = useMemo(
    () => ({
      ...
      confirmKey: (keyId, confirmed) =>
        api.post("/admin/key/confirm", { keyId, confirmed }),
      deleteKey: (keyId) => api.post("/admin/key/remove", { keyId }),
      listDatasets: () => api.get("/admin/dataset/list"),
      deleteDataset: (mnemonic) =>
        api.post(`/admin/dataset/${mnemonic}/remove`),
      destroyDataset: (mnemonic) =>
        api.post(`/admin/dataset/${mnemonic}/destroy`),
      recoverDataset: (mnemonic) =>
        api.post(`/admin/dataset/${mnemonic}/recover`),
  ...
  const contextValue = useMemo(
    () => ({
      ...
      uploadChunk: async (chunk, mnemonic) => {
        const { start, end, hash, data, totalSize } = chunk;
      ...
      fetchKey: async (mnemonic, keyHash) =>
        api.post(`/dataset/${mnemonic}/key`, { keyHash }),
      removeDataset: async (mnemonic) =>
        api.post(`/dataset/${mnemonic}/remove`),
      renameDataset: async (mnemonic, name) =>
        api.post(`/dataset/${mnemonic}/rename`, { name }),
      addDatasetMembers: async (mnemonic, members, key) =>
        api.post(`/dataset/${mnemonic}/member/add`, { members, key }),
      setMemberAccess: async (mnemonic, user, permission) =>
        api.post(`/dataset/${mnemonic}/member/set`, { user, permission }),
      reencryptDataset: async (mnemonic, key) =>
        api.post(`/dataset/${mnemonic}/reencrypt`, { key }),
      fetchDataset: async (mnemonic) => api.get(`/dataset/${mnemonic}`),
      fetchChunk: async (mnemonic, chunkId) =>
        api.get(`/dataset/${mnemonic}/chunk/${chunkId}`, {
          responseType: "blob",
        }),
    }),
    [api, session, admin]
