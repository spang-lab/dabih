...
pub async fn chunk_upload(configuration: &configuration::Configuration, mnemonic: &str, content_range: &str, digest: &str, chunk: Option<std::path::PathBuf>) -> Result<models::Chunk, Error<ChunkUploadError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_mnemonic = mnemonic;
