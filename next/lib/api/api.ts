...
const init = (baseUrl: string) => {
  ...
  const user = {
    ...
    me: async () => {
      // Fix horrible openapi-fetch behavior
      // that sets data to {} when response status is 204
      const result = await c.GET("/user/me");
  ...
  const dataset = {
    get: (mnemonic: string) =>
      c.GET("/dataset/{mnemonic}", { params: { path: { mnemonic } } }),
    search: (body: schemas["SearchRequestBody"]) =>
      c.POST("/dataset/search", { body }),
    rename: (mnemonic: string, name: string) =>
      c.POST("/dataset/{mnemonic}/rename", {
        params: { path: { mnemonic } },
        body: { name },
      }),
    remove: (mnemonic: string) =>
      c.POST("/dataset/{mnemonic}/remove", { params: { path: { mnemonic } } }),
    restore: (mnemonic: string) =>
      c.POST("/dataset/{mnemonic}/restore", { params: { path: { mnemonic } } }),
    destroy: (mnemonic: string, force?: boolean) =>
      c.POST("/dataset/{mnemonic}/destroy", {
        params: { path: { mnemonic } },
        body: { force: force ?? false },
      }),
    addMember: (mnemonic: string, body: schemas["MemberAddBody"]) =>
      c.POST("/dataset/{mnemonic}/addMember", {
        params: { path: { mnemonic } },
        body,
      }),
    setAccess: (mnemonic: string, body: schemas["SetAccessBody"]) =>
      c.POST("/dataset/{mnemonic}/setAccess", {
        params: { path: { mnemonic } },
        body,
      }),
  };
  ...
  const download = {
    ...
    chunk: (mnemonic: string, hash: string) =>
      c.GET("/download/{mnemonic}/chunk/{hash}", {
        parseAs: "stream",
